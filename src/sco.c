...
static void *sco_dispatcher_thread(struct ba_adapter *a) {
	...
	for (;;) {
		...
		ba_transport_thread_send_signal(t->sco.spk_pcm.th, BA_TRANSPORT_SIGNAL_BT_RELEASE);
		ba_transport_thread_send_signal(t->sco.mic_pcm.th, BA_TRANSPORT_SIGNAL_BT_RELEASE);
		...
		ba_transport_thread_send_signal(t->sco.spk_pcm.th, BA_TRANSPORT_SIGNAL_BT_ACQUIRE);
		ba_transport_thread_send_signal(t->sco.mic_pcm.th, BA_TRANSPORT_SIGNAL_BT_ACQUIRE);
...
void *sco_thread(struct ba_transport_thread *th) {
	...
	for (ba_transport_thread_set_state_running(th);;) {
		...
		if (pfds[0].revents & POLLIN) {
			...
			case BA_TRANSPORT_SIGNAL_BT_ACQUIRE:
				ba_transport_thread_bt_acquire(th);
				continue;
			case BA_TRANSPORT_SIGNAL_BT_RELEASE:
